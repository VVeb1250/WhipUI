...
test('init creates Claude Code instructions and a local WhipUI skill', async () => {
  ...
  assert.match(claudeInstructions, /WhipUI \+ WhipDesign/)
  assert.match(claudeInstructions, /UI\/UX Pro Max/)
  assert.match(claudeSkill, /Playwright MCP/)
  assert.match(claudeSkill, /Impeccable/)
  const designSkill = await readFile(join(projectRoot, '.claude/skills/whipdesign/SKILL.md'), 'utf8')
  assert.match(designSkill, /name: whipdesign/)
  assert.match(claudeInstructions, /\.whipui\/router\.md/)
  assert.match(claudeSkill, /workflows\/implement\.md/)
})
